...
-- | Same as onChange, however use the description of the second property
-- as the description of the combined property.
trigger ::
  (IsProp x, IsProp y, Combines x y) =>
  x ->
  y ->
  CombinedType x y
trigger x y = do
  let x' = setDesc x $ getDesc y
  onChange x' y
