...
instance ChrootBootstrapper Debootstrapped where
  buildchroot (Debootstrapped cf) info loc = case system of
    ...
    (Just (System (CentOS _) _)) -> Left "CentOS Linux not supported by debootstrap."
...
provisioned' c@(Chroot loc bootstrapper infopropigator _) systemdonly caps =
  ...
  where
    ...
    setup =
      propellChroot c (inChrootProcess (not systemdonly) c) systemdonly caps
        `requires` built
...
chrootInfo (Chroot loc _ _ h) =
  mempty
    `addInfo` mempty {_chroots = M.singleton loc h}
...
inChrootProcess keepprocmounted (Chroot loc _ _ _) cmd = do
  ...
  where
    ...
    mountproc =
      unlessM (elem procloc <$> mountPointsBelow loc) $
    ...
    cleanup
      ...
      | otherwise =
        whenM (elem procloc <$> mountPointsBelow loc) $
          umountLazy procloc
...
exposeTrueLocaldir a =
  ifM
    (hasContainerCapability FilesystemContained)
