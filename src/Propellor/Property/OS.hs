...
shadowConfigForInstallDebian :: Property DebianLike
shadowConfigForInstallDebian = tightenTargets $ User.shadowConfig True
...
cleanInstallOnce confirmation =
  check (not <$> doesFileExist flagfile) $
  ...
  where
    ...
    flipped = property (newOSDir ++ " moved into place") $
      liftIO $ do
        ...
        renamesout <-
          map (\d -> (d, oldOSDir ++ d, pure $ d `notElem` (oldOSDir : newOSDir : trickydirs)))
            <$> dirContents "/"
        renamesin <-
          map (\d -> let dest = "/" ++ takeFileName d in (d, dest, not <$> fileExist dest))
    ...
    propellorbootstrapped =
      property "propellor re-debootstrapped in new os" $
        return NoChange
...
oldOSRemoved confirmation =
  check (doesDirectoryExist oldOSDir) $
