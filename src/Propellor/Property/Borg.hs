...
import Utility.SafeCommand (toCommand)
...
restored dir repo = go `requires` installed
  where
    ...
    restore latest = withTmpDirIn (takeDirectory dir) "borg-restore" $ \tmpdir -> do
      ...
      ifM
        (pure ok <&&> doesDirectoryExist restoreddir)
        ( do
...
backup dir repo crontimes extraargs kp =
  backup' dir repo crontimes extraargs kp
...
backup' dir repo crontimes extraargs kp =
  ...
  where
    ...
    cronjob =
      Cron.niceJob ("borg_backup" ++ dir) crontimes (User "root") "/" $
