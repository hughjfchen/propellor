...
type CentOS = MetaTypes '[ 'Targeting 'OSCentOS]

-- | CentOS and derivatives.
type CentOSLike = MetaTypes '[ 'Targeting 'OSCentOS]

-- | NonDebianLike, currently include CentOSLike and ArchLinux
type NonDebianLike = ArchLinux + CentOSLike
...
data instance Sing (x :: MetaType) where
  ...
  OSCentOSS :: Sing ('Targeting 'OSCentOS)
...
instance SingI ('Targeting 'OSCentOS) where sing = OSCentOSS
...
instance SingKind ('KProxy :: KProxy MetaType) where
  ...
  fromSing OSCentOSS = Targeting OSCentOS
...
-- mistake.
  CannotCombine list1 list2 ('Just note) =
    TypeError
      ( 'Text "Cannot combine two Properties."
...
type family PrettyPrintMetaType t :: ErrorMessage where
  ...
  PrettyPrintMetaType ('Targeting 'OSCentOS) = 'ShowType CentOS
