...
interactiveInit =
  ifM
    (doesDirectoryExist =<< dotPropellor)
...
fullClone = do
  ...
  ok <-
    ifM
      (doesFileExist distrepo <||> doesDirectoryExist distrepo)
      ( allM
          id
...
updateUpstreamMaster newref = do
  ...
  where
    ...
    git = run "git"
    run cmd ps =
      unlessM (boolSystem cmd (map Param ps)) $
