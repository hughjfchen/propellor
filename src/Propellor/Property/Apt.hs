...
suiteAvailablePinned s pin = available <!> unavailable
  where
    ...
    setSourcesFile = tightenTargets $
      withMirror (desc True) $ \u ->
...
pinnedTo ::
  ...
  RevertableProperty Debian Debian
pinnedTo ps pins =
  mconcat (map (\p -> pinnedTo' p pins) ps)
...
unattendedUpgrades = enable <!> disable
  where
    ...
    configure =
      propertyList "unattended upgrades configured" $
        props
          & enableupgrading
          ...
          case o of
            ...
            (Just (System (Debian _ suite) _))
              | not (isStable suite) ->
                ensureProperty w $
                  unattendedconfig
                    `File.containsLine` ("Unattended-Upgrade::Origins-Pattern { \"o=Debian,a=" ++ showSuite suite ++ "\"; };")
            _ -> noChange
...
periodicUpdates =
  tightenTargets $
    "/etc/apt/apt.conf.d/02periodic"
      `File.hasContent` [ "APT::Periodic::Enable \"1\";",
        ...
        if null vals
          then noChange
            ...
            withHandle
              ...
              (proc "debconf-set-selections" [])
              $ \h -> do
                forM_ vals $ \(tmpl, tmpltype, value) ->
                  hPutStrLn h $ unwords [package, tmpl, tmpltype, value]
                hClose h
    reconfigure =
      cmdPropertyEnv "dpkg-reconfigure" ["-fnone", package] noninteractiveEnv
        `assume` MadeChange
...
trustsKey' k = check (not <$> doesFileExist f) $
  property desc $
    makeChange $ do
      withHandle
        StdinHandle
        createProcessSuccess
        (proc "apt-key" ["--keyring", f, "add", "-"])
        $ \h -> do
          hPutStr h (pubkey k)
          hClose h
...
noPDiffs =
  tightenTargets $
    "/etc/apt/apt.conf.d/20pdiffs"
      `File.hasContent` ["Acquire::PDiffs \"false\";"]
...
proxy' u =
  tightenTargets $
    "/etc/apt/apt.conf.d/20proxy"
      `File.hasContent` ["Acquire::HTTP::Proxy \"" ++ u ++ "\";"]
      `describe` desc
  where
