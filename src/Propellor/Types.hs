...
type family TightenTargetsAllowed untightened tightened :: Constraint where
  TightenTargetsAllowed untightened tightened =
    If
      ( Targets tightened `IsSubset` Targets untightened
...
-- try to ensure y.
instance SingI metatypes => Sem.Semigroup (Property (MetaTypes metatypes)) where
...
instance SingI metatypes => Monoid (Property (MetaTypes metatypes)) where
