  ...
  where
    go w (Just hostos)
      | supportsArch hostos targetarch && not (useEmulation config) =
        ensureProperty w $
          built'
            (setupRevertableProperty installed)
            target
            system
            config
    go w _ = ensureProperty w $ do
      let p =
            setupRevertableProperty foreignBinariesEmulated
...
built' installprop target system@(System _ arch) config =
  ...
  where
    go =
      check (isUnpopulated target <||> ispartial) setupprop
        `requires` installprop

    setupprop :: Property Linux
    setupprop = property ("debootstrapped " ++ target) $
      liftIO $ do
        ...
        ifM
          (boolSystemEnv cmd params (Just de))
          ( return MadeChange,
            return FailedChange
          )
    ...
    ispartial =
      ifM
        (doesDirectoryExist (target </> "debootstrap"))
        ( do
            removeChroot target
            return True,
          return False
...
extractSuite (System (CentOS _) _) = Nothing
...
perlInstalled =
  check (not <$> inPath "perl") $
    property "perl installed" $
      liftIO $
        toResult . isJust
          <$> firstM
            id
...
sourceInstall' = withTmpDir "debootstrap" $ \tmpd -> do
  ...
  urls <-
    sortBy (flip compare) -- highest version first
...
sourceRemove =
  property "debootstrap not installed from source" $
    liftIO $
      ifM
        (doesDirectoryExist sourceInstallDir)
        ( do
