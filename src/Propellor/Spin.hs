...
import Network.Socket (AddrInfo (..), AddrInfoFlag (..), SockAddr, defaultHints, getAddrInfo)
...
commitSpin = do
  ...
  void $
    actionMessage "Git commit" $
      gitCommit
        (Just spinCommitMessage)
  ...
  whenM hasOrigin $
    void $
      actionMessage "Push to central git repository" $
...
updateServer target relay hst connect haveprecompiled privdata = do
  (Just toh, Just fromh, _, pid) <-
    createProcessNonConcurrent $
      connect
        { std_in = CreatePipe,
          std_out = CreatePipe
        }
...
sendPrivData hn toh privdata = void $
  actionMessage msg $ do
...
sendGitUpdate hn fromh toh =
  void $
    actionMessage ("Sending git update to " ++ hn) $ do
      sendMarked toh gitPushMarker ""
      (Nothing, Nothing, Nothing, h) <- createProcess p
      (==) ExitSuccess <$> waitForProcess h
  where
    p =
      (proc "git" ["upload-pack", "."])
...
sendGitClone hn = void $
  actionMessage ("Clone git repository to " ++ hn) $ do
    branch <- getCurrentBranch
    cacheparams <- sshCachingParams hn
    withTmpFile "propellor.git" $ \tmp _ ->
      allM
        id
...
sendPrecompiled hn = void $
  actionMessage "Uploading locally compiled propellor as a last resort" $
...
mergeSpin = do
  ...
  where
    rungit cmd ps =
      unlessM (boolSystem "git" (Param cmd : ps)) $
