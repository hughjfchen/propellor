...
bootstrapWith b = pureInfoProperty desc (InfoVal b)
  where
    desc =
      "propellor bootstrapped with " ++ case b of
...
bootstrappedFrom reposource =
  check (hasContainerCapability FilesystemContained) $
...
clonedFrom reposource = case reposource of
  ...
  where
    go :: Property Linux
    go =
      property ("Propellor repo cloned from " ++ sourcedesc) $
    ...
    needclone =
      (hasContainerCapability FilesystemContained <&&> truelocaldirisempty)
